...
currentCourses: any[] = []; // قائمة الدورات الحالية التي ستعرض في الصفحة الحالية
first: number = 0; // تحديد قيمة افتراضية لـ first
rows: number = 3; // عدد الصفوف في الصفحة

constructor() {
  this.loadCourses(); // لتحميل الدورات الأولية عند تهيئة المكون
}

loadCourses() {
  // يتم استدعاء هذه الدالة عند تغيير الصفحة أو عند تهيئة المكون لتحميل الدورات الحالية
  this.currentCourses = this.courses.slice(this.first, this.first + this.rows);
}

onPageChange(event: PaginatorState) {
  // يتم استدعاء هذه الدالة عندما يتم تغيير الصفحة
  this.first = event.first || 0; // التحقق من أن event.first غير معرفة
  this.rows = event.rows || 0;
  this.loadCourses(); // إعادة تحميل الدورات الحالية بعد تغيير الصفحة
}
